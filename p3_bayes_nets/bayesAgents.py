...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):  
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVar_factor = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assignment in obsVar_factor.getAllPossibleAssignmentDicts():

                ghost_house = assignment[GHOST_HOUSE_VAR]
                food_house = assignment[FOOD_HOUSE_VAR]
                color = assignment[obsVar]

                manhattan = lambda x,y: abs(x[0]-y[0])+abs(x[1]-y[1])
                spec = [BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL]
                adjacent_pos = sorted([(manhattan(obsPos,v),spec[i]) for i,v in enumerate(gameState.getPossibleHouses())])[0][1]
                
                if adjacent_pos != ghost_house and adjacent_pos != food_house:
                    prob = 1 if color == "none" else 0
                elif adjacent_pos == ghost_house and ghost_house != food_house:
                    prob = PROB_GHOST_RED if color == RED_OBS_VAL else (1-PROB_GHOST_RED)
                    prob = 0 if color == 'none' else prob
                else:
                    prob = PROB_FOOD_RED if color == RED_OBS_VAL else (1-PROB_FOOD_RED)
                    prob = 0 if color == 'none' else prob
                obsVar_factor.setProbability(assignment, prob)
            bayesNet.setCPT(obsVar, obsVar_factor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    P = inferenceByVariableElimination(bayesNet, queryVariables, evidence, eliminationOrder)
    D = sorted([(P.getProbability(i),i) for i in P.getAllPossibleAssignmentDicts()], key = lambda x:x[0])
    return D[-1][1]
